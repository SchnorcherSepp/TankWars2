...
func (g *Game) writeGlobalText(screen *ebiten.Image) {
	...
	// write list
	for _, su := range core.ShadowUnitsSortList {

		// calc score
		if !su.Destroyed {
			if su.Unit.Player == ai2000.PLAYER {
				myScore += su.Unit.Score()
			} else {
				otherScore += su.Unit.Score()
			}
		}
		...
		// write line
		s += fmt.Sprintf("   [%d]: %s %s(%d)\tLastSeen: %d\n", su.Unit.Player, status, string(su.Unit.Type), su.Unit.Health, su.LastSeen)
	}

	// write score
	if myScore+otherScore != 0 {
		s += fmt.Sprintf("\n      [me] %d vs. %d [other]\n", myScore*100/(myScore+otherScore), otherScore*100/(myScore+otherScore))
	}
